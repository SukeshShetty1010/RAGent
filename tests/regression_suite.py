...
from retriever.evaluation_runner import (
    EvaluationRunner,
    TestCase,
    EvaluationResult,
)
...
REGRESSION_VAULT: List[RegressionCase] = [
    ...
    RegressionCase(
        ...
        test_case=TestCase(
            ...
            required_structure_pattern=re.compile(
                r"knowledgeable gaming assistant", re.S
            ),
        ),
    ),
]
...
class RegressionRunner:
    ...
    def run(self) -> bool:
        ...
        for case in REGRESSION_VAULT:
            ...
            if result is None:
                print(f"{RED}❌ FAILED: No evaluation result returned{RESET}\n")
                all_passed = False
                continue
    ...
    def _run_case(self, case: RegressionCase) -> EvaluationResult | None:
    ...
    def _compare(
        ...
        """
        Compare expected TestCase constraints with actual EvaluationResult.
        """
        ...
        if not actual.web_trigger_accuracy:
            diffs.append(
                f"Web trigger behavior incorrect "
