...
RerankerProvider = Literal["local", "voyage"]

VALID_PROVIDERS = ("local", "voyage")

# "local" = in-process fastembed cross-encoder (Xenova/ms-marco-MiniLM-L-6-v2,
# raw logits). "voyage" = Voyage's rerank HTTP API (normalized 0..1). The
# two produce incomparable score scales — see quality_gate.py's _FLOORS.
